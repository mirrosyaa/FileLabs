...
function ConversionPage({
  ...
  console.log(
    "ConversionPage render - isConverting:",
    isConverting,
    "fadeIn:",
    fadeIn,
    "progress:",
        ...
        <div className={styles.conversionBox}>
          ...
          {hasMixedTypes && (
            <div className={styles.warningBox}>
              Mixed file types detected. Please upload one type at a time.
            </div>
          )}

          {!hasMixedTypes &&
            primaryFileType &&
            primaryFileType !== "unknown" && (
              <>
                {getConversionOptions(primaryFileType, files).length > 0 ? (
                  <div className={styles.formatOptions}>
                    <div className={styles.formatButtons}>
                      {getConversionOptions(primaryFileType, files).map(
                        (format) => (
                          <button
                            key={format.value}
                            className={`${styles.formatOption} ${
                              selectedFormat === format.value
                                ? styles.selectedFormat
                                : ""
                            }`}
